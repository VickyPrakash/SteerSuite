...
Util::Vector SocialForcesAgent::wallFollower(SteerLib::AgentGoalInfo goalInfo, Util::Vector goalDirection) {
	...
	Util::Vector goal = Util::Vector(0.0f, 0.0f, 0.0f);;
	std::set<SteerLib::SpatialDatabaseItemPtr> _neighbors;
	...
	if (_neighbors.size() == 0)
	{
		...
		for (std::set<SteerLib::SpatialDatabaseItemPtr>::iterator neighbor = _neighbors.begin(); neighbor != _neighbors.end(); neighbor++)
		{
			...
			if (!(*neighbor)->isAgent()) {
				...
				if (normalize(cross(calcWallNormal(wall), cross(calcWallNormal(wall), n))) != goal)
				{
					goalDirection = normalize(cross(calcWallNormal(wall), cross(calcWallNormal(wall), n)));
					goal = normalize(calcWallNormal(wall));
...
Util::Vector SocialForcesAgent::pursueEvade(SteerLib::AgentGoalInfo goalInfo, Util::Vector goalDirection) {
	std::set<SteerLib::SpatialDatabaseItemPtr> _neighbors;
...
void SocialForcesAgent::updateAI(float timeStamp, float dt, unsigned int frameNumber)
{
	...
	goalDirection = pursueEvade(goalInfo, goalDirection);
